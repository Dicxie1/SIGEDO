...
namespace Asistencia.Hubs
{
    public class QuizHub : Hub
    {
        ...
        public async Task JoinAsStudent(string pin, string nickname)
        {
            ...
            if (session != null )
            {
                ...
                if(session != null && session.CurrentQuestionIndex >= 0)
                {
                    ...
                    var tiemeElapsed = (DateTime.UtcNow - session.CurrentQuestionStartTime).TotalSeconds;
                    var remainingTime = currentQuestion.TimeLimitSecond - (int)tiemeElapsed;
                    if(remainingTime > 0)
                    {
                        ...
                        var questionData = new
                        {
                            ...
                            answer = currentQuestion.AnserOptions?.FirstOrDefault(a => a.IsCorrect == true)?.Text,
                        ...
                        await Clients.Clients(Context.ConnectionId).SendAsync("ReceiveQuestion", questionData);
                    }
                    else
                    {
                        await Clients.Clients(Context.ConnectionId).SendAsync("ShowWaitingResults");
                    }
        ...
        public async Task SubmitAnswer(string pin, string nickname, int optionIndex, double timeElapsedSeconds)
        {
            ...
            await Clients.Group($"{pin}_Teacher").SendAsync("AnswerReceived", isCorrect);
        }
